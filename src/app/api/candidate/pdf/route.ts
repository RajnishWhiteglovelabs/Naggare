...
function ascii(s?: string | null): string {
  ...
  return String(s)
    ...
    .split('').map(ch => { const cc = ch.charCodeAt(0); return (cc > 0xFF || (cc >= 0x80 && cc <= 0x9F)) ? '' : ch }).join('')
    .replace(/\s+/g, ' ').trim()
}
...
function fmtDate(d?: string): string {
  ...
  if (p.length === 2) return `${m[parseInt(p[1]) - 1] || ''} ${p[0]}`
  return p[0]
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    function career(entries: any[]) {
      section('Career Journey')
      const dotX = M + 4, textX = M + 20; const centers: number[] = []
      for (let i = 0; i < entries.length; i++) {
        const e = entries[i], cur = i === 0
        const dateStr = [fmtDate(e.from), e.to ? fmtDate(e.to) : (e.from ? 'Present' : '')].filter(Boolean).join(' - ')
        ensure(42)
        const cy = y - 6; centers.push(cy)
        const dd = cur ? 9 : 7
        page.drawSvgPath(rr(dd, dd, dd / 2), { x: dotX - dd / 2, y: cy + dd / 2, color: indigo })
        page.drawText(ascii(e.org), { x: textX, y: y - 9, size: 11, font: bold, color: cur ? indigo : dark }); y -= 13
        if (e.role) { page.drawText(ascii(e.role), { x: textX, y: y - 8, size: 9.5, font, color: gray }); y -= 12 }
        if (dateStr) { page.drawText(dateStr, { x: textX, y: y - 8, size: 8.5, font, color: rgb(0.6, 0.63, 0.68) }); y -= 11 }
        y -= 9
      }
      if (centers.length > 1) page.drawLine({ start: { x: dotX, y: centers[0] }, end: { x: dotX, y: centers[centers.length - 1] }, thickness: 1, color: indigo, opacity: 0.3 })
    }

    // Order matches the on-screen profile: Career -> Details -> Skills -> In their own words -> What they're looking for
    const careerList = Array.isArray(c.career) ? c.career.filter((e: any) => e && e.org) : []
    if (careerList.length) career(careerList)

    const details: string[] = []
    if (c.availability) details.push(c.availability)
    if (c.notice_period) details.push('Notice: ' + c.notice_period)
    if (c.work_preference) details.push(c.work_preference)
    if (c.current_ctc) details.push('Current: Rs. ' + c.current_ctc + ' LPA')
    if (c.expected_ctc) details.push('Expected: Rs. ' + c.expected_ctc + ' LPA')
    ...
    y -= 16; ensure(16)
    page.drawText('Generated via Naggare  \u00B7  naggare.com', { x: M, y: y - 4, size: 8, font, color: gray })
