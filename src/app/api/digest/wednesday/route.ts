...
import { emailRecruiterHumpDay, emailCandidateHumpDay } from '@/lib/emails'
  ...
  try {
    ...
    if (recruiters) {
      for (const r of recruiters) {
        const { count: pendingChats } = await db.from('chat_sessions').select('*', { count: 'exact', head: true })
          .eq('recruiter_email', r.email).eq('status','pending').lt('expires_at', now)
        const { subject, html } = emailRecruiterHumpDay(r.name, pendingChats || 0)
    ...
    if (candidates) {
      for (const c of candidates) {
        const { subject, html } = emailCandidateHumpDay(c.name)
