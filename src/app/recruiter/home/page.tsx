...
export default function RecruiterHome() {
  ...
  const [filterCurrentCtcMin, setFilterCurrentCtcMin] = useState('')
  ...
  // Filtered candidates
  const activeFilterCount = [filterDomain, filterExp, filterAvailability, filterNotice, filterWork, filterCity, filterCurrentCtcMin, filterCurrentCtcMax, filterExpectedCtcMin, filterExpectedCtcMax].filter(Boolean).length
  const filteredCandidates = candidates.filter(cand => {
    ...
    const matchesCurrentCtc = (() => {
      if (!filterCurrentCtcMin && !filterCurrentCtcMax) return true
      const ctc = parseFloat(cand.current_ctc || '0')
      if (filterCurrentCtcMin && ctc < parseFloat(filterCurrentCtcMin)) return false
      if (filterCurrentCtcMax && ctc > parseFloat(filterCurrentCtcMax)) return false
      return true
    })()
    const matchesExpectedCtc = (() => {
      if (!filterExpectedCtcMin && !filterExpectedCtcMax) return true
      const ctc = parseFloat(cand.expected_ctc || '0')
      if (filterExpectedCtcMin && ctc < parseFloat(filterExpectedCtcMin)) return false
      if (filterExpectedCtcMax && ctc > parseFloat(filterExpectedCtcMax)) return false
      return true
    })()
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: '#F5F3FF' }}>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {/* CANDIDATE EXPANDED MODAL - shared component (same card everywhere) */}
        {expandedCandidate && (
          <CandidateProfileCard
            candidate={expandedCandidate}
            score={candidateScores[expandedCandidate.email]}
